...
class obv():
    ...
    def entry_exit_base(self):
        self.entryprofo = {'OBV': {'HIGH': 0.5, 'LOW': -0.5}}
        return self.entryprofo

    def Check_Entry(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['OBV']
        self.entryba = self.tech_dict['OBV']['LOW']
    ...
    def Check_Exit(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['OBV']
        self.entryba = self.tech_dict['OBV']['HIGH']
        if (float(self.item) > float(self.entryba)):
            return True
        else:
            return False
    ...
    def tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['OBV'] = self.lineedit.text()
    ...
    def calculate(self):
        ...
        self.datadb["OBV"] = talib.OBV(
            self.datadb["Close"], self.datadb["Volume"])
    ...
    def Entry_tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['OBV'] = {}
        self.tool_dict['OBV']['HIGH'] = self.EntryHighlineedit.text()
        self.tool_dict['OBV']['LOW'] = self.EntryLowlineedit.text()
        return self.tool_dict
