...
class rocp():
    ...
    def entry_exit_base(self):
        self.entryprofo = {'ROCP': {'HIGH': 0.2, 'LOW': -0.2}}
        return self.entryprofo

    def setup(self):
        tech_dict = self.getterTechValue()
        if 'ROCP' in tech_dict:
            self.data = tech_dict['ROCP']
            return self.data
        else:
            self.datadef = 10
            return self.datadef
    ...
    def Check_Entry(self, testitem):
        ...
        self.item = testitem.loc['ROCP']
        self.entryba = self.tech_dict['ROCP']['LOW']
    ...
    def Check_Exit(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['ROCP']
        self.entryba = self.tech_dict['ROCP']['HIGH']
        if (float(self.item) > float(self.entryba)):
            return True
        else:
            return False
    ...
    def tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['ROCP'] = self.lineedit.text()
    ...
    def calculate(self):
        ...
        self.datadb["ROCP"] = talib.ROCP(self.datadb["Close"], timeperiod=int(self.parameter))
    ...
    def Entry_tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['ROCP'] = {}
        self.tool_dict['ROCP']['HIGH'] = self.EntryHighlineedit.text()
        self.tool_dict['ROCP']['LOW'] = self.EntryLowlineedit.text()
        return self.tool_dict
