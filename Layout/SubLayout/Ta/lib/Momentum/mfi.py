...
class mfi():
    ...
    def Check_Entry(self, testitem):
        ...
        self.item = testitem.loc['MFI']
        self.entryba = self.tech_dict['MFI']['LOW']
    ...
    def Check_Exit(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['MFI']
        self.entryba = self.tech_dict['MFI']['HIGH']
        if (float(self.item) > float(self.entryba)):
            return True
        else:
            return False
    ...
    def tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['MFI'] = self.lineedit.text()
    ...
    def calculate(self):
        ...
        self.datadb["MFI"] = talib.MFI(
            self.datadb["High"], self.datadb["Low"], self.datadb["Close"], self.datadb["Volume"], timeperiod=int(self.parameter))
    ...
    def Entry_tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['MFI'] = {}
        self.tool_dict['MFI']['HIGH'] = self.EntryHighlineedit.text()
        self.tool_dict['MFI']['LOW'] = self.EntryLowlineedit.text()
        return self.tool_dict
