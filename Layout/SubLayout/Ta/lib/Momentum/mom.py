...
class mom():
    ...
    def entry_exit_base(self):
        self.entryprofo = {'MOM': {'HIGH': 20, 'LOW': -20}}
        return self.entryprofo
    
    def Check_Entry(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['MOM']
        self.entryba = self.tech_dict['MOM']['LOW']
    ...
    def Check_Exit(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['MOM']
        self.entryba = self.tech_dict['MOM']['HIGH']
        if (float(self.item) >= float(self.entryba)):
            return True
        else:
            return False
    ...
    def tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['MOM'] = self.lineedit.text()
    ...
    def calculate(self):
        ...
        self.datadb["MOM"] = talib.MOM(self.datadb["Close"], timeperiod=int(self.parameter))
    ...
    def Entry_tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['MOM'] = {}
        self.tool_dict['MOM']['HIGH'] = self.EntryHighlineedit.text()
        self.tool_dict['MOM']['LOW'] = self.EntryLowlineedit.text()
        return self.tool_dict
