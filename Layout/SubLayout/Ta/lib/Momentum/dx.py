...
class dx():
    ...
    def entry_exit_base(self):
        self.entryprofo = {'DX': {'HIGH': 100, 'LOW': 0}}
        return self.entryprofo
    
    def Check_Entry(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['DX']
        self.entryba = self.tech_dict['DX']['LOW']
    ...
    def Check_Exit(self, testitem):
        self.tech_dict = self.getterEntryTechValue()
        self.item = testitem.loc['DX']
        self.entryba = self.tech_dict['DX']['HIGH']
        if (float(self.item) >= float(self.entryba)):
            return True
        else:
            return False
    ...
    def tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['DX'] = self.lineedit.text()
    ...
    def Entry_tool_dicts(self):
        self.tool_dict = {}
        self.tool_dict['DX'] = {}
        self.tool_dict['DX']['HIGH'] = self.EntryHighlineedit.text()
        self.tool_dict['DX']['LOW'] = self.EntryLowlineedit.text()
        return self.tool_dict
